...
Principios para el uso responsable de IA en el equipo de calidad de PetTech.
*   **Consentimiento:** Es obligatorio contar con el consentimiento explícito del equipo antes de procesar datos reales con herramientas de IA. No usar datos de producción en prompts.
*   **GitHub Copilot:** Herramienta principal para asistencia en escritura de código de automatización, generación de casos Gherkin y análisis de specs. Usar con el framework ASDD definido en `.github/`.
*   **Agentes ASDD:** Usar los agentes especializados (`@QA Agent`, `/gherkin-case-generator`, `/risk-identifier`) para tareas repetibles de QA. Siempre validar el output generado antes de aceptarlo.
*   **Revisión humana obligatoria:** Todo artefacto generado por IA (casos de prueba, Gherkin, análisis de riesgos) debe ser revisado y validado por un ingeniero de QA antes de usarse como fuente de verdad.
*   **Trazabilidad:** Mantener trazabilidad entre los artefactos de QA generados por IA y la spec de origen (`SPEC-ID` en `.github/specs/`).
