...
2. **Backend architecture is Clean Architecture** — `interfaces/ → domain/ → infrastructure/` — follow the pattern defined in `.github/instructions/backend.instructions.md`. Never bypass layers.
3. **No ORM queries in views.py** — delegar siempre a repositories en `infrastructure/`.
4. **UI state follows the project architecture** — usar `useAuthStore` (Zustand) como única fuente de verdad. No duplicar estado de autenticación.
5. **Django ORM is synchronous** — no usar patrones async/await de FastAPI. Las vistas DRF son síncronas por defecto.
6. **Never commit secrets or credentials** — `.env`, credential files y API keys deben estar en `.gitignore`.
